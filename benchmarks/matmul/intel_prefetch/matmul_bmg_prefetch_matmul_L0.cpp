...
int main() {
    ...
    // Tensor argument: a (rank=2, float, 4194304 elements, compact)
    float* a_ptr = nullptr;
    result = zeMemAllocShared(context, &deviceDesc, &hostDesc,
        4194304 * sizeof(float), 1, device, (void**)&a_ptr);
    if (result != ZE_RESULT_SUCCESS) {
        std::cerr << "ERROR: zeMemAllocShared failed for a" << std::endl;
        return 1;
    }
    for (size_t _i = 0; _i < 4194304; _i++) a_ptr[_i] = (float)(_i % 5);
    // Arg 0: a PTR
    zeKernelSetArgumentValue(kernel, 0, sizeof(void*), &a_ptr);
    // Arg 1: a BYTE_SIZE = 16777216
    uint64_t a_byte_size = 16777216ULL;
    ...
    // Tensor argument: b (rank=2, float, 4194304 elements, compact)
    float* b_ptr = nullptr;
    result = zeMemAllocShared(context, &deviceDesc, &hostDesc,
        4194304 * sizeof(float), 1, device, (void**)&b_ptr);
    if (result != ZE_RESULT_SUCCESS) {
        std::cerr << "ERROR: zeMemAllocShared failed for b" << std::endl;
        return 1;
    }
    for (size_t _i = 0; _i < 4194304; _i++) b_ptr[_i] = (float)(_i % 3);
    // Arg 9: b PTR
    zeKernelSetArgumentValue(kernel, 9, sizeof(void*), &b_ptr);
    // Arg 10: b BYTE_SIZE = 16777216
    uint64_t b_byte_size = 16777216ULL;
    ...
    { int mma_ok = 1; int mma_bad = 0;
      for (uint64_t i = 0; i < 2048ULL; i++) for (uint64_t j = 0; j < 2048ULL; j++) {
        float acc = 0.0f;
