 ...
 * Source: /workspace/benchmarks/matmul/intel_prefetch/matmul_bmg_prefetch_matmul.metacrisp
 * Kernel: matmul
 *
 * Compilation:
...
int main() {
    ...
    // Load SPIR-V module
    const char* spv_path = "/workspace/benchmarks/matmul/intel_prefetch/matmul_bmg_prefetch.spv";
    ...
    // Tensor argument: a (rank=2, float, 1048576 elements, compact)
    float* a_ptr = nullptr;
    result = zeMemAllocShared(context, &deviceDesc, &hostDesc,
        1048576 * sizeof(float), 1, device, (void**)&a_ptr);
    if (result != ZE_RESULT_SUCCESS) {
        std::cerr << "ERROR: zeMemAllocShared failed for a" << std::endl;
        return 1;
    }
    for (size_t _i = 0; _i < 1048576; _i++) a_ptr[_i] = (float)(_i % 5);
    // Arg 0: a PTR
    zeKernelSetArgumentValue(kernel, 0, sizeof(void*), &a_ptr);
    // Arg 1: a BYTE_SIZE = 4194304
    uint64_t a_byte_size = 4194304ULL;
    ...
    // Tensor argument: b (rank=2, float, 1048576 elements, compact)
    float* b_ptr = nullptr;
    result = zeMemAllocShared(context, &deviceDesc, &hostDesc,
        1048576 * sizeof(float), 1, device, (void**)&b_ptr);
    if (result != ZE_RESULT_SUCCESS) {
        std::cerr << "ERROR: zeMemAllocShared failed for b" << std::endl;
        return 1;
    }
    for (size_t _i = 0; _i < 1048576; _i++) b_ptr[_i] = (float)(_i % 3);
    // Arg 9: b PTR
    zeKernelSetArgumentValue(kernel, 9, sizeof(void*), &b_ptr);
    // Arg 10: b BYTE_SIZE = 4194304
    uint64_t b_byte_size = 4194304ULL;
    ...
    ze_device_properties_t _deviceProps = { ZE_STRUCTURE_TYPE_DEVICE_PROPERTIES };
    zeDeviceGetProperties(device, &_deviceProps);
    uint32_t _hw_threads = _deviceProps.numSlices * _deviceProps.numSubslicesPerSlice * _deviceProps.numEUsPerSubslice * _deviceProps.numThreadsPerEU;
    // Refine with kernel resource footprint (privateMemSize, spillMemSize)
    ze_kernel_properties_t _kernelProps = { ZE_STRUCTURE_TYPE_KERNEL_PROPERTIES };
    zeKernelGetProperties(kernel, &_kernelProps);
    // Derate occupancy by 2x if kernel spilled to private memory
    if (_kernelProps.spillMemSize > 0) { _hw_threads /= 2; }
    // :derive-from tensor 'C' (length=c_length) used for check-thread-bounds; grid uses occupancy
    ...
    { int mma_ok = 1; int mma_bad = 0;
      for (uint64_t i = 0; i < 1024ULL; i++) for (uint64_t j = 0; j < 1024ULL; j++) {
        float acc = 0.0f;
