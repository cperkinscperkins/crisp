 ...
 * Source: /workspace/benchmarks/matmul/sec2_top_bf16/matmul_bmg_bf16_matmul.metacrisp
 * Kernel: matmul
 *
 * Compilation:
...
int main() {
    ...
    // Tensor argument: a (rank=2, uint16_t, 67108864 elements, compact)
    uint16_t* a_ptr = nullptr;
    result = zeMemAllocShared(context, &deviceDesc, &hostDesc,
        67108864 * sizeof(uint16_t), 1, device, (void**)&a_ptr);
    if (result != ZE_RESULT_SUCCESS) {
        std::cerr << "ERROR: zeMemAllocShared failed for a" << std::endl;
        return 1;
    }
    for (size_t _i = 0; _i < 67108864; _i++) a_ptr[_i] = crisp_f32_to_bf16((float)(_i % 5));
    // Arg 0: a PTR
    zeKernelSetArgumentValue(kernel, 0, sizeof(void*), &a_ptr);
    // Arg 1: a BYTE_SIZE = 134217728
    uint64_t a_byte_size = 134217728ULL;
    ...
    // Arg 4: a STRIDE_0 = 8192
    uint64_t a_str0 = 8192ULL;
    ...
    // Arg 6: a EXTENT_0 = 8192
    uint64_t a_ext0 = 8192ULL;
    zeKernelSetArgumentValue(kernel, 6, sizeof(uint64_t), &a_ext0);
    // Arg 7: a EXTENT_1 = 8192
    uint64_t a_ext1 = 8192ULL;
    zeKernelSetArgumentValue(kernel, 7, sizeof(uint64_t), &a_ext1);
    // Arg 8: a LENGTH = 67108864
    uint64_t a_length = 67108864ULL;
    zeKernelSetArgumentValue(kernel, 8, sizeof(uint64_t), &a_length);


    // Tensor argument: b (rank=2, uint16_t, 67108864 elements, compact)
    uint16_t* b_ptr = nullptr;
    result = zeMemAllocShared(context, &deviceDesc, &hostDesc,
        67108864 * sizeof(uint16_t), 1, device, (void**)&b_ptr);
    if (result != ZE_RESULT_SUCCESS) {
        std::cerr << "ERROR: zeMemAllocShared failed for b" << std::endl;
        return 1;
    }
    for (size_t _i = 0; _i < 67108864; _i++) b_ptr[_i] = crisp_f32_to_bf16((float)(_i % 3));
    // Arg 9: b PTR
    zeKernelSetArgumentValue(kernel, 9, sizeof(void*), &b_ptr);
    // Arg 10: b BYTE_SIZE = 134217728
    uint64_t b_byte_size = 134217728ULL;
    ...
    // Arg 13: b STRIDE_0 = 8192
    uint64_t b_str0 = 8192ULL;
    ...
    // Arg 15: b EXTENT_0 = 8192
    uint64_t b_ext0 = 8192ULL;
    zeKernelSetArgumentValue(kernel, 15, sizeof(uint64_t), &b_ext0);
    // Arg 16: b EXTENT_1 = 8192
    uint64_t b_ext1 = 8192ULL;
    zeKernelSetArgumentValue(kernel, 16, sizeof(uint64_t), &b_ext1);
    // Arg 17: b LENGTH = 67108864
    uint64_t b_length = 67108864ULL;
    zeKernelSetArgumentValue(kernel, 17, sizeof(uint64_t), &b_length);


    // Tensor argument: c (rank=2, float, 67108864 elements, compact)
    float* c_ptr = nullptr;
    result = zeMemAllocShared(context, &deviceDesc, &hostDesc,
        67108864 * sizeof(float), 1, device, (void**)&c_ptr);
    if (result != ZE_RESULT_SUCCESS) {
        std::cerr << "ERROR: zeMemAllocShared failed for c" << std::endl;
        return 1;
    }
    memset(c_ptr, 0, 67108864 * sizeof(float));
    // Arg 18: c PTR
    zeKernelSetArgumentValue(kernel, 18, sizeof(void*), &c_ptr);
    // Arg 19: c BYTE_SIZE = 268435456
    uint64_t c_byte_size = 268435456ULL;
    ...
    // Arg 22: c STRIDE_0 = 8192
    uint64_t c_str0 = 8192ULL;
    ...
    // Arg 24: c EXTENT_0 = 8192
    uint64_t c_ext0 = 8192ULL;
    zeKernelSetArgumentValue(kernel, 24, sizeof(uint64_t), &c_ext0);
    // Arg 25: c EXTENT_1 = 8192
    uint64_t c_ext1 = 8192ULL;
    zeKernelSetArgumentValue(kernel, 25, sizeof(uint64_t), &c_ext1);
    // Arg 26: c LENGTH = 67108864
    uint64_t c_length = 67108864ULL;
    ...
    // --mma-bench: per-launch kernel-timestamp timing (Endeavor 142; fixed 143)
    {
        ...
        double _flops = 2.0 * 8192.0 * 8192.0 * 8192.0;
        double _gflops = (_med_us > 0.0) ? (_flops / (_med_us / 1e6) / 1e9) : 0.0;
        std::cout << "BENCH 8192 8192 8192 " << _gflops << " GFLOPS (" << _kn << " iters, " << _total_s << " s)"
    ...
    { int mma_ok = 1; int mma_bad = 0;
      uint64_t chk_m = (8192ULL < 64 ? 8192ULL : 64); uint64_t chk_n = (8192ULL < 64 ? 8192ULL : 64);
