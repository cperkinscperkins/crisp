/*
 * CUTLASS 3.x Hopper TMA warp-specialized bf16 GEMM — the PEER for §2.2 (NVIDIA).
 *
 * Twin of sec2_top_fp16/cutlass_peer_fp16.cu; the ONLY intended difference is ElementA/ElementB
 * (cutlass::bfloat16_t vs cutlass::half_t).  If you change one, change both.
 *
 ...
 * chapter is that NO ONE KERNEL FITS ALL N -- established on Intel, where the bf16 chapter
 ...
 *
 * Build: nvcc -O3 -std=c++17 -arch=sm_90a -I<cutlass>/include cutlass_peer_bf16.cu -o <bin>
...
int main(int argc, char** argv) {
    ...
    // OPERAND bf16, ACCUMULATOR f32, C f32 — the same computation cuBLAS is asked for in
    // cublas_ceiling_bf16.cu and the same one Crisp's tensor-core path performs.  Alignment is
    ...
    auto bail = [&](const char* where, cutlass::Status st) {
        fprintf(stderr, "cutlass_peer_bf16 [%s]: %s FAILED (%s) at M=%d N=%d K=%d — this config "
    ...
    /* FAIL LOUDLY.  See sec2_top/cutlass_peer.cu for the full history: a build without the
       ...
       VISIBLE GAP. */
    fprintf(stderr, "cutlass_peer_bf16 [%s]: CUTLASS headers not found at build time — this "
