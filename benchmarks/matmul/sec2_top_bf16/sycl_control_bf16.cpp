/*
 * SYCL Joint Matrix BFloat16 Control reference for BattleMage.
 *
 * Implements a 32x32 register-ring tiled kernel using:
 *  - sycl::ext::oneapi::experimental::matrix::joint_matrix (XMX BF16 DPAS)
 ...
 *
 * Build: icpx -fsycl -O3 -Xs "-ze-opt-large-register-file" sycl_control_bf16.cpp -o sycl_control_bf16
 */
...
int main(int argc, char** argv) {
        ...
        return q.submit([&](sycl::handler& h) {
            h.parallel_for(sycl::nd_range<1>(global, local), [=](sycl::nd_item<1> item)
                ...
                if (tile_m < n_tiles_m && tile_n < n_tiles_n) {
                    ...
                    using tA_t = joint_matrix<sycl::sub_group, bfloat16, use::a, 8, 16, layout::row_major>;
                    using tB_t = joint_matrix<sycl::sub_group, bfloat16, use::b, 16, 16, layout::ext_intel_packed>;
    ...
    printf("{\n  \"algorithm\": \"matmul_top_bf16\",\n  \"implementation\": \"sycl_control_bf16\",\n");
    printf("  \"N\": %d, \"M\": %d, \"K\": %d,\n", N, M, K);
