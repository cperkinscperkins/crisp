 ...
 * Source: /workspace/benchmarks/matmul/sec3_mma_lowering/matmul_coop_tuned_matmul.metacrisp
 * Kernel: matmul
 *
 * Compilation:
 *   g++ -o launcher matmul_coop_tuned_matmul_L0.cpp -I<path-to-level-zero>/include -lze_loader
 *
...
int main() {
    ...
    // Load SPIR-V module
    const char* spv_path = "/workspace/benchmarks/matmul/sec3_mma_lowering/matmul_coop_tuned.spv";
    ...
    // Tensor argument: a (rank=2, uint16_t, 1048576 elements, compact)
    uint16_t* a_ptr = nullptr;
    result = zeMemAllocShared(context, &deviceDesc, &hostDesc,
        1048576 * sizeof(uint16_t), 1, device, (void**)&a_ptr);
    if (result != ZE_RESULT_SUCCESS) {
        std::cerr << "ERROR: zeMemAllocShared failed for a" << std::endl;
        return 1;
    }
    for (size_t _i = 0; _i < 1048576; _i++) a_ptr[_i] = crisp_f32_to_bf16((float)(_i % 5));
    // Arg 0: a PTR
    zeKernelSetArgumentValue(kernel, 0, sizeof(void*), &a_ptr);
    // Arg 1: a BYTE_SIZE = 2097152
    uint64_t a_byte_size = 2097152ULL;
    ...
    // Arg 4: a STRIDE_0 = 1024
    uint64_t a_str0 = 1024ULL;
    ...
    // Arg 6: a EXTENT_0 = 1024
    uint64_t a_ext0 = 1024ULL;
    zeKernelSetArgumentValue(kernel, 6, sizeof(uint64_t), &a_ext0);
    // Arg 7: a EXTENT_1 = 1024
    uint64_t a_ext1 = 1024ULL;
    zeKernelSetArgumentValue(kernel, 7, sizeof(uint64_t), &a_ext1);
    // Arg 8: a LENGTH = 1048576
    uint64_t a_length = 1048576ULL;
    zeKernelSetArgumentValue(kernel, 8, sizeof(uint64_t), &a_length);


    // Tensor argument: b (rank=2, uint16_t, 1048576 elements, compact)
    uint16_t* b_ptr = nullptr;
    result = zeMemAllocShared(context, &deviceDesc, &hostDesc,
        1048576 * sizeof(uint16_t), 1, device, (void**)&b_ptr);
    if (result != ZE_RESULT_SUCCESS) {
        std::cerr << "ERROR: zeMemAllocShared failed for b" << std::endl;
        return 1;
    }
    for (size_t _i = 0; _i < 1048576; _i++) b_ptr[_i] = crisp_f32_to_bf16((float)(_i % 3));
    // Arg 9: b PTR
    zeKernelSetArgumentValue(kernel, 9, sizeof(void*), &b_ptr);
    // Arg 10: b BYTE_SIZE = 2097152
    uint64_t b_byte_size = 2097152ULL;
    ...
    // Arg 13: b STRIDE_0 = 1024
    uint64_t b_str0 = 1024ULL;
    ...
    // Arg 15: b EXTENT_0 = 1024
    uint64_t b_ext0 = 1024ULL;
    zeKernelSetArgumentValue(kernel, 15, sizeof(uint64_t), &b_ext0);
    // Arg 16: b EXTENT_1 = 1024
    uint64_t b_ext1 = 1024ULL;
    zeKernelSetArgumentValue(kernel, 16, sizeof(uint64_t), &b_ext1);
    // Arg 17: b LENGTH = 1048576
    uint64_t b_length = 1048576ULL;
    zeKernelSetArgumentValue(kernel, 17, sizeof(uint64_t), &b_length);


    // Tensor argument: c (rank=2, float, 1048576 elements, compact)
    float* c_ptr = nullptr;
    result = zeMemAllocShared(context, &deviceDesc, &hostDesc,
        1048576 * sizeof(float), 1, device, (void**)&c_ptr);
    if (result != ZE_RESULT_SUCCESS) {
        std::cerr << "ERROR: zeMemAllocShared failed for c" << std::endl;
        return 1;
    }
    memset(c_ptr, 0, 1048576 * sizeof(float));
    // Arg 18: c PTR
    zeKernelSetArgumentValue(kernel, 18, sizeof(void*), &c_ptr);
    // Arg 19: c BYTE_SIZE = 4194304
    uint64_t c_byte_size = 4194304ULL;
    ...
    // Arg 22: c STRIDE_0 = 1024
    uint64_t c_str0 = 1024ULL;
    ...
    // Arg 24: c EXTENT_0 = 1024
    uint64_t c_ext0 = 1024ULL;
    zeKernelSetArgumentValue(kernel, 24, sizeof(uint64_t), &c_ext0);
    // Arg 25: c EXTENT_1 = 1024
    uint64_t c_ext1 = 1024ULL;
    zeKernelSetArgumentValue(kernel, 25, sizeof(uint64_t), &c_ext1);
    // Arg 26: c LENGTH = 1048576
    uint64_t c_length = 1048576ULL;
    ...
    { int mma_ok = 1; int mma_bad = 0;
      uint64_t chk_m = (1024ULL < 64 ? 1024ULL : 64); uint64_t chk_n = (1024ULL < 64 ? 1024ULL : 64);
      ...
      for (uint64_t i = 0; i < chk_m; i++) for (uint64_t j = 0; j < chk_n; j++) {
        float acc = 0.0f;
        for (uint64_t kk = 0; kk < 1024ULL; kk++)
