/*
 * Hand-written CUDA fp16 matmul — the CONTROL for §2.1 (NVIDIA).
 *
 ...
 * Keeping the algorithm fixed is what makes the tf32 -> fp16 ratio in the report attributable to
 ...
 * the fix is a __half2 (4-byte) staging variant as a SECOND control, not an edit to this one --
...
typedef __half elem_t;
__device__ __forceinline__ float to_f32(elem_t x) { return __half2float(x); }
