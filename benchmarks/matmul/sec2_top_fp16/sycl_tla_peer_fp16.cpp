...
int main(int argc, char const **argv) {
    try {
        ...
        // queue, in the f16 encoding of ElementInputA/B.  A wrong encoding fails verification.
        {
            sycl::queue fq = compat::get_default_queue();
            crisp_bench::sycl_fill_encoded(fq, block_A.get(), size_t(M) * K, crisp_bench::FILL_MOD_A, "f16");
            crisp_bench::sycl_fill_encoded(fq, block_B.get(), size_t(K) * N, crisp_bench::FILL_MOD_B, "f16");
