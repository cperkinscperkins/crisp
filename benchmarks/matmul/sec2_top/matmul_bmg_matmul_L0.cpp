 ...
 * Source: /workspace/benchmarks/matmul/sec2_top/matmul_bmg_matmul.metacrisp
 * Kernel: matmul
 *
 * Compilation:
...
int main() {
    ...
    // Load SPIR-V module
    const char* spv_path = "/workspace/benchmarks/matmul/sec2_top/matmul_bmg.spv";
    ...
    // Tensor argument: a (rank=2, float, 67108864 elements, compact)
    float* a_ptr = nullptr;
    result = zeMemAllocShared(context, &deviceDesc, &hostDesc,
        67108864 * sizeof(float), 1, device, (void**)&a_ptr);
    if (result != ZE_RESULT_SUCCESS) {
        std::cerr << "ERROR: zeMemAllocShared failed for a" << std::endl;
        return 1;
    }
    for (size_t _i = 0; _i < 67108864; _i++) a_ptr[_i] = (float)(_i % 5);
    // Arg 0: a PTR
    zeKernelSetArgumentValue(kernel, 0, sizeof(void*), &a_ptr);
    // Arg 1: a BYTE_SIZE = 268435456
    uint64_t a_byte_size = 268435456ULL;
    ...
    // Tensor argument: b (rank=2, float, 67108864 elements, compact)
    float* b_ptr = nullptr;
    result = zeMemAllocShared(context, &deviceDesc, &hostDesc,
        67108864 * sizeof(float), 1, device, (void**)&b_ptr);
    if (result != ZE_RESULT_SUCCESS) {
        std::cerr << "ERROR: zeMemAllocShared failed for b" << std::endl;
        return 1;
    }
    for (size_t _i = 0; _i < 67108864; _i++) b_ptr[_i] = (float)(_i % 3);
    // Arg 9: b PTR
    zeKernelSetArgumentValue(kernel, 9, sizeof(void*), &b_ptr);
    // Arg 10: b BYTE_SIZE = 268435456
    uint64_t b_byte_size = 268435456ULL;
