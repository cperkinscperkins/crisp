/*
 * Chapter 5 (Endeavor 150) — NVIDIA baseline for an activation the vendor CANNOT fuse.
 *
...
__global__ void relu_pass(float* C, size_t n) {
    size_t i = (size_t)blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) { float v = C[i]; C[i] = v > 0.0f ? v : 0.0f; }
}
...
int main(int argc, char** argv) {
    ...
    auto relu = [&]() { relu_pass<<<rgrid, rblock>>>(dC, total); };
