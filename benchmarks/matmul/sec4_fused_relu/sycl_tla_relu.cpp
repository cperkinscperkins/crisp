/*
 * SYCL-TLA Peer Contender for Fused Epilogue (ReLU).
 * Implements a 32x32 register-ring XMX DPAS kernel with fused ReLU epilogue.
 *
 * Build: icpx -fsycl -O3 -Xs "-ze-opt-large-register-file" sycl_tla_relu.cpp -o sycl_tla_relu
 * Run:   ./sycl_tla_relu [M] [N] [K] [warmup] [iters]
 */
...
int main(int argc, char** argv) {
    try {
            ...
            return q.submit([&](sycl::handler& h) {
                h.parallel_for(
                    ...
                    [=](sycl::nd_item<2> item) {
                        ...
                        // Epilogue with fused ReLU: store to memory with max(val, 0)
                        for (int r = 0; r < 4; ++r) {
                            for (int c_idx = 0; c_idx < 2; ++c_idx) {
        ...
        for (size_t i = 0; i < (size_t)M * N; i++) {
            float val = C[i];
            float exp_val = expected > 0.0f ? (float)expected : 0.0f;
            maxerr = std::max(maxerr, (double)std::fabs(val - exp_val));
        }
