/*
 * SYCL-TLA Peer Contender for Fused ReLU Epilogue (§4) on Intel BattleMage (Arc B580).
 ...
 *   - CollectiveEpilogue (IntelXeGeneric, LinCombEltAct<ReLu>)
...
int main(int argc, char const **argv) {
    try {
        ...
        if (gemm_op.can_implement(arguments) != cutlass::Status::kSuccess) {
            std::cerr << "SYCL-TLA cannot implement Fused ReLU for size: " << M << "x" << N << "x" << K << std::endl;
            return 1;
        }
        ...
        const crisp_bench::VerifyResult vr = crisp_bench::sycl_verify(vq, block_D.get(), (uint64_t)M, (uint64_t)N, (uint64_t)K, [](double x) { return x > 0.0 ? x : 0.0; });
        ...
        printf("{\n  \"algorithm\": \"matmul_relu\",\n  \"implementation\": \"sycl_tla_relu\",\n");
        printf("  \"N\": %d, \"M\": %d, \"K\": %d,\n", N, M, K);
        ...
        std::cerr << "SYCL-TLA ReLU exception: " << e.what() << std::endl;
        return 1;
    }
}
