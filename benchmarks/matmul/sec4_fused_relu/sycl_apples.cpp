 ...
 * simply false.)
 *
 * ACTIVATION: plain relu — matches the Crisp twin and CUBLASLT_EPILOGUE_RELU semantics.
 */
...
int main(int argc, char** argv) {
                ...
                [=](sycl::nd_item<2> item) {
                    ...
                    for (int r = 0; r < 4; ++r)
                        for (int c_idx = 0; c_idx < 2; ++c_idx) {
                            ...
                            joint_matrix_apply(sg, c[r][c_idx], [](float &v) { v = v > 0.0f ? v : 0.0f; });
    ...
    const crisp_bench::VerifyResult vr = crisp_bench::sycl_verify(q, C, (uint64_t)M, (uint64_t)N, (uint64_t)K, [](double x) { return x > 0.0 ? x : 0.0; });
    ...
    printf("{\n  \"algorithm\": \"matmul\",\n  \"implementation\": \"sycl+relu-fused\",\n");
    printf("  \"N\": %d, \"M\": %d, \"K\": %d,\n", N, M, K);
