/*
 * Hand-written CUDA reference matmul for Chapter 1 (Async Linear).
 * Uses cuda::pipeline and cuda::memcpy_async to asynchronously load the tiles
 * into shared memory before waiting and computing.
 */
...
__global__ void matmul_tiled_async(const float* A, const float* B, float* C, int M, int N, int K) {
    ...
    // We use a single-stage pipeline (linear async load)
...
int main(int argc, char** argv) {
    ...
    auto launch = [&](){ matmul_tiled_async<<<grid,block>>>(dA,dB,dC,M,N,K); };
