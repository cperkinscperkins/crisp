/*
 * Hand-written SYCL reference matmul using Intel XMX joint_matrix with direct global load.
 * Section 1 Chapter 4 Control baseline for Intel.
 * Register-resident load directly from global memory to XMX fragments (single-stage, unpipelined).
 *
            ...
            h.parallel_for(
                ...
                [=](sycl::nd_item<2> it) {
                    ...
                    for (int k = 0; k < K; k += 8) {
                        tA_t a[4];
                        for (int r = 0; r < 4; r++)
                            joint_matrix_load(sg, a[r], make_ptr(A + (row + r * 8) * K + k), K);

                        tB_t b[2];
                        for (int c_idx = 0; c_idx < 2; c_idx++)
                            joint_matrix_load(sg, b[c_idx], make_ptr(B + k * N + (col + c_idx * 16)), N);
