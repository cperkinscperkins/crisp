/*
 * Chapter 6 (Endeavor 150) — APPLES-TO-APPLES fused epilogue, SYCL / Intel BMG.
 *
 * intel_prefetch/sycl_apples.cpp (joint_matrix XMX + joint_matrix_prefetch — the mirror of
 * matmul_bmg_prefetch.crisp) with the activation applied to each accumulator fragment via
 * joint_matrix_apply before the store.
 *
 * WHY joint_matrix_apply IS THE RIGHT CONTENDER.  It is SYCL's own elementwise-over-a-fragment
 * operation — the direct analog of Crisp's (map-elements! C-tile #'f).  So this is the SAME
 * algorithm with the SAME class of mechanism, hand-written; whatever gap remains is Crisp's
 * abstraction cost, which is the only thing an apples-to-apples column should measure.
 *
 * (The first cut used the chap1-era 16x16 shared-memory kernel here — not a mirror of the
 * prefetch algorithm at all.  It read ~1.4 TF against Crisp's ~24 and the "apples" label was
 * simply false.)
 *
 * ACTIVATION: deliberately ARBITRARY — v > 0.5 ? v : v*v * 0.01.  No more
 * expensive than relu (a compare and two FMAs, no transcendentals), so the ONLY thing that
 * changes versus the relu variant is that no vendor library has it on a menu.  That is the
 * entire point of the +CUSTOM column.
 *
 * ORACLE-PRESERVING BY CONSTRUCTION: the harness gates on A=B=1 => C == K, so the POSITIVE
 * branch must be the identity.  The first version used (v-0.5)*1.25, which maps 256 -> 319.375;
 * the Crisp twin's sweep was discarded outright and this one reported numbers that had failed
 * validation.  Only the sub-threshold branch is transformed now.
 */
...
int main(int argc, char** argv) {
        ...
        return q.submit([&](sycl::handler& h) {
            h.parallel_for(
                ...
                [=](sycl::nd_item<2> item) {
                    ...
                    for (int r = 0; r < 4; ++r)
                        for (int c_idx = 0; c_idx < 2; ++c_idx) {
                            // The fused epilogue: SYCL's own elementwise-over-a-fragment op,
                            // the direct analog of Crisp's (map-elements! C-tile #'f).
                            joint_matrix_apply(sg, c[r][c_idx], [](float &v) { v = v > 0.5f ? v : v * v * 0.01f; });
    ...
    printf("{\n  \"algorithm\": \"matmul\",\n  \"implementation\": \"sycl+custom-fused\",\n");
    printf("  \"N\": %d, \"M\": %d, \"K\": %d,\n", N, M, K);
