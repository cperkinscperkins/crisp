/*
 * Chapter 5 (Endeavor 150) — APPLES-TO-APPLES fused epilogue, SYCL / Intel BMG.
 *
 ...
 * simply false.)
 *
 * ACTIVATION: deliberately ARBITRARY — v > 0.5 ? (v - 0.5) * 1.25 : v * 0.01.  No more
                ...
                [=](sycl::nd_item<2> item) {
                    ...
                    for (int r = 0; r < 4; ++r)
                        for (int c_idx = 0; c_idx < 2; ++c_idx) {
                            ...
                            joint_matrix_apply(sg, c[r][c_idx], [](float &v) { v = v > 0.5f ? (v - 0.5f) * 1.25f : v * 0.01f; });
