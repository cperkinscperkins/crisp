/*
 * Chapter 6 (Endeavor 150) — cuBLASLt WHEN IT CANNOT FUSE.
 *
...
int main(int argc, char** argv) {
    ...
    // NO epilogue: cuBLASLt's epilogues are a FIXED ENUM and a quadratic tail is not in it,
    // so the activation cannot ride inside the matmul and becomes its own pass over C.
    ...
    const int RB = 256;
    dim3 ablock(RB), agrid((unsigned)((total + RB - 1) / RB));
    auto launch = [&]() {
        LK(cublasLtMatmul(lt, op, &alpha, dA, Adesc, dB, Bdesc, &beta,
                          dC, Cdesc, dC, Cdesc, &heur.algo, dWs, wsBytes, 0));
        act_pass<<<agrid, ablock>>>(dC, total);   // same stream: ordered after the matmul
    };
    ...
    const crisp_bench::VerifyResult vr = crisp_bench::cuda_verify(dC, "f32", (uint64_t)M, (uint64_t)N, (uint64_t)K, crisp_bench::cm(M), crisp_bench::cm(K), crisp_bench::cm(M), [](double x) { return x > 0.5 ? x : x * x * 0.01; });
    ...
    printf("{\n  \"algorithm\": \"matmul\",\n  \"implementation\": \"cublaslt+custom\",\n");
    printf("  \"N\": %d, \"M\": %d, \"K\": %d,\n", N,M,K);
