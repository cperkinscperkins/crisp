 ...
 * Source: /workspace/benchmarks/matmul/sec4_fused_custom/matmul_bmg_prefetch_custom_matmul.metacrisp
 * Kernel: matmul
 *
 * Compilation:
 *   g++ -o launcher matmul_bmg_prefetch_custom_matmul_L0.cpp -I<path-to-level-zero>/include -lze_loader
 *
...
int main() {
    ...
    // Load SPIR-V module
    const char* spv_path = "/workspace/benchmarks/matmul/sec4_fused_custom/matmul_bmg_prefetch_custom.spv";
    ...
    // Tensor argument: a (rank=2, float, 268435456 elements, compact)
    float* a_ptr = nullptr;
    result = zeMemAllocShared(context, &deviceDesc, &hostDesc,
        268435456 * sizeof(float), 1, device, (void**)&a_ptr);
    if (result != ZE_RESULT_SUCCESS) {
        std::cerr << "ERROR: zeMemAllocShared failed for a" << std::endl;
        return 1;
    }
    for (size_t _i = 0; _i < 268435456; _i++) a_ptr[_i] = (float)(_i % 5);
    // Arg 0: a PTR
    zeKernelSetArgumentValue(kernel, 0, sizeof(void*), &a_ptr);
    // Arg 1: a BYTE_SIZE = 1073741824
    uint64_t a_byte_size = 1073741824ULL;
    ...
    // Arg 4: a STRIDE_0 = 16384
    uint64_t a_str0 = 16384ULL;
    ...
    // Arg 6: a EXTENT_0 = 16384
    uint64_t a_ext0 = 16384ULL;
    zeKernelSetArgumentValue(kernel, 6, sizeof(uint64_t), &a_ext0);
    // Arg 7: a EXTENT_1 = 16384
    uint64_t a_ext1 = 16384ULL;
    zeKernelSetArgumentValue(kernel, 7, sizeof(uint64_t), &a_ext1);
    // Arg 8: a LENGTH = 268435456
    uint64_t a_length = 268435456ULL;
    zeKernelSetArgumentValue(kernel, 8, sizeof(uint64_t), &a_length);


    // Tensor argument: b (rank=2, float, 268435456 elements, compact)
    float* b_ptr = nullptr;
    result = zeMemAllocShared(context, &deviceDesc, &hostDesc,
        268435456 * sizeof(float), 1, device, (void**)&b_ptr);
    if (result != ZE_RESULT_SUCCESS) {
        std::cerr << "ERROR: zeMemAllocShared failed for b" << std::endl;
        return 1;
    }
    for (size_t _i = 0; _i < 268435456; _i++) b_ptr[_i] = (float)(_i % 3);
    // Arg 9: b PTR
    zeKernelSetArgumentValue(kernel, 9, sizeof(void*), &b_ptr);
    // Arg 10: b BYTE_SIZE = 1073741824
    uint64_t b_byte_size = 1073741824ULL;
    ...
    // Arg 13: b STRIDE_0 = 16384
    uint64_t b_str0 = 16384ULL;
    ...
    // Arg 15: b EXTENT_0 = 16384
    uint64_t b_ext0 = 16384ULL;
    zeKernelSetArgumentValue(kernel, 15, sizeof(uint64_t), &b_ext0);
    // Arg 16: b EXTENT_1 = 16384
    uint64_t b_ext1 = 16384ULL;
    zeKernelSetArgumentValue(kernel, 16, sizeof(uint64_t), &b_ext1);
    // Arg 17: b LENGTH = 268435456
    uint64_t b_length = 268435456ULL;
    zeKernelSetArgumentValue(kernel, 17, sizeof(uint64_t), &b_length);


    // Tensor argument: c (rank=2, float, 268435456 elements, compact)
    float* c_ptr = nullptr;
    result = zeMemAllocShared(context, &deviceDesc, &hostDesc,
        268435456 * sizeof(float), 1, device, (void**)&c_ptr);
    if (result != ZE_RESULT_SUCCESS) {
        std::cerr << "ERROR: zeMemAllocShared failed for c" << std::endl;
        return 1;
    }
    memset(c_ptr, 0, 268435456 * sizeof(float));
    // Arg 18: c PTR
    zeKernelSetArgumentValue(kernel, 18, sizeof(void*), &c_ptr);
    // Arg 19: c BYTE_SIZE = 1073741824
    uint64_t c_byte_size = 1073741824ULL;
    ...
    // Arg 22: c STRIDE_0 = 16384
    uint64_t c_str0 = 16384ULL;
    ...
    // Arg 24: c EXTENT_0 = 16384
    uint64_t c_ext0 = 16384ULL;
    zeKernelSetArgumentValue(kernel, 24, sizeof(uint64_t), &c_ext0);
    // Arg 25: c EXTENT_1 = 16384
    uint64_t c_ext1 = 16384ULL;
    zeKernelSetArgumentValue(kernel, 25, sizeof(uint64_t), &c_ext1);
    // Arg 26: c LENGTH = 268435456
    uint64_t c_length = 268435456ULL;
    ...
    // --mma-bench: per-launch kernel-timestamp timing (Endeavor 142; fixed 143)
    {
        ...
        double _flops = 2.0 * 16384.0 * 16384.0 * 16384.0;
        double _gflops = (_med_us > 0.0) ? (_flops / (_med_us / 1e6) / 1e9) : 0.0;
        std::cout << "BENCH 16384 16384 16384 " << _gflops << " GFLOPS (" << _kn << " iters, " << _total_s << " s)"
    ...
    if (268435456 <= 512) {
        std::cout << "BUFFER a: ";
        for (size_t i = 0; i < 268435456; i++) {
            std::cout << a_ptr[i] << (i == 268435456 - 1 ? "" : " ");
        }
        std::cout << std::endl;
    }
    if (268435456 <= 512) {
        std::cout << "BUFFER b: ";
        for (size_t i = 0; i < 268435456; i++) {
            std::cout << b_ptr[i] << (i == 268435456 - 1 ? "" : " ");
        }
        std::cout << std::endl;
    }
    if (268435456 <= 512) {
        std::cout << "BUFFER c: ";
        for (size_t i = 0; i < 268435456; i++) {
            std::cout << c_ptr[i] << (i == 268435456 - 1 ? "" : " ");
        }
        std::cout << std::endl;
    }
    ...
    { int mma_ok = 1; int mma_bad = 0;
      for (uint64_t i = 0; i < (16384ULL < 64 ? 16384ULL : 64); i++) for (uint64_t j = 0; j < (16384ULL < 64 ? 16384ULL : 64); j++) {
        float acc = 0.0f;
        for (uint64_t kk = 0; kk < 16384ULL; kk++)
