/*
 * SYCL-TLA Peer Contender for Fused Custom Activation Epilogue (§4) on Intel BattleMage (Arc B580).
 * Built with the genuine Intel SYCL-TLA (CUTLASS 3.x / CuTe for SYCL) library.
 *
 * Implements:
 *   - Custom C++ epilogue activation functor: f(x) = (x > 0.5) ? x : (x * x * 0.01)
 *   - CuTe TiledMMA (XE_DPAS_TT<8, float, bfloat16_t>)
 *   - CollectiveMMA (MainloopXeL1Staged<2>) with 2-stage prefetch
 *   - CollectiveEpilogue (IntelXeGeneric, LinCombEltAct<CustomActivationFunctor>)
...
// Custom activation functor matching Crisp's custom-act function
template <typename T>
struct CustomActivationFunctor {
    ...
    T operator()(T const &scalar) const {
        return (scalar > T(0.5f)) ? scalar : (scalar * scalar * T(0.01f));
    }
};

template <typename T, int N>
struct CustomActivationFunctor<cutlass::Array<T, N>> {
    static const bool kIsHeavy = false;

    CUTLASS_HOST_DEVICE
    cutlass::Array<T, N> operator()(cutlass::Array<T, N> const &rhs) const {
        cutlass::Array<T, N> y;
        CustomActivationFunctor<T> op;
...
// Fused Custom Functor Epilogue Op
using EpilogueOp = cutlass::epilogue::fusion::LinCombEltAct<
    CustomActivationFunctor,
...
int main(int argc, char const **argv) {
    try {
        ...
        if (gemm_op.can_implement(arguments) != cutlass::Status::kSuccess) {
            std::cerr << "SYCL-TLA cannot implement Fused Custom for size: " << M << "x" << N << "x" << K << std::endl;
            return 1;
        }
