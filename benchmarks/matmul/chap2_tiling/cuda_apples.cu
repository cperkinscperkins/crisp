 ...
 * A = B = 1.0 so every C[i][j] == K. Standard 16x16 shared-memory tiled kernel (fp32).
 * Section 1 Chapter 2 Control baseline for NVIDIA.
 *
...
int main(int argc, char** argv) {
    ...
    float k_med=kt[iters/2] * 1000.0f; // ms -> us
    float k_min=kt[0] * 1000.0f;
