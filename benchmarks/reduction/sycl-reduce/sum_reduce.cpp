/*
 * SYCL sum reduction benchmark — library-primitive variant.
 *
 * Sibling of ../sycl/sum_reduce.cpp: same algorithm shape (grid-stride
 * accumulate → workgroup reduce → one atomic per workgroup), but the
 * workgroup reduction phase uses sycl::reduce_over_group instead of an
 * explicit SLM tree loop.
 *
 * This is the SYCL analog of using cub::BlockReduce in a CUDA harness:
 *   - sycl/      (hand-written explicit SLM tree)   ↔ NVIDIA cuda/
 *   - sycl-reduce (library primitive)               ↔ NVIDIA cub/
 *
 * (oneDPL's oneapi::dpl::reduce was the old library tier — but oneDPL is
 * really a std::algorithm-style wrapper, not a CUB-equivalent primitive.
 * reduce_over_group sits at the right level for the comparison.)
 *
 * Launch policy mirrors Crisp's :strategy :strided :occupancy R for L0:
 *   global = (numEUs * occupancy) workgroups × blockSize work-items
 *
 * Usage: ./sum_reduce_sycl_reduce [N] [warmup] [iterations] [occupancy]
 *   occupancy = 0.0..1.0 ratio for grid sizing (default: 1.0 = max).
 *
 * Output: JSON to stdout — same schema as the CUDA / Crisp / sycl harnesses.
 *
 * Compile (inside Docker container with oneAPI):
 *   icpx -fsycl -O3 sum_reduce.cpp -o sum_reduce_sycl_reduce
 */
...
int main(int argc, char** argv) {
    ...
    auto launch = [&]() {
        return q.submit([&](sycl::handler& h) {
            ...
            h.parallel_for(sycl::nd_range<1>(sycl::range<1>(global),
                ...
                // Phase 1: grid-stride accumulate into register (same as
                ...
                // Phase 2: workgroup reduce via library primitive.
                // sycl::reduce_over_group dispatches to the platform-
                // optimal cooperative reduction (subgroup intrinsics +
                // workgroup-level cleanup on Intel iGPU/Arc).  Replaces
                // the explicit SLM tree-reduce in the hand-written impl.
                float wg_sum = sycl::reduce_over_group(item.get_group(),
                                                       acc,
                                                       sycl::plus<float>());

                // Phase 3: one atomic-add per workgroup (same as hand-written).
    ...
    std::printf("  \"implementation\": \"sycl-reduce\",\n");
