    ...
    (let ((canonical-spec (%scratch-tensor-canonical-spec op args)))
      (when canonical-spec
        (let* ((binding-name (or (compiler-context-current-binding-name *compiler-context*)
          ...
          ;; Endeavor 137: record the tile's box shape keyed by (fn . binding) so a later :block
          ;; load-tile of this tile (possibly in a sub-function it's passed to) can capture the
          ;; CUtensorMap box-dims for its descriptor.  canonical-spec = (tensor ELEM N ...).
          (when (and size-expr (consp canonical-spec))
            (let ((dims (if (listp size-expr) size-expr (list size-expr))))
              (setf (gethash (cons fn-name binding-name) *scratch-tile-dims*)
    ...
    (unless same-elem
      ...
      (when (%mv-is-struct-elem src-elem)
        (error 'crisp-compiler-error
               :message (format nil "~a: Cannot reinterpret a struct-element storage handle to a different element type ~a (source element type ~a is a struct)" op new-elem src-elem)
               :source-location location))
      ;; Restriction 2: source alignment cannot be :strided
      (when (member src-align '(:strided strided) :test #'string-equal)
        (error 'crisp-compiler-error
               :message (format nil "~a: Cannot reinterpret element type on a :strided storage handle (source is ~a, new type is ~a). Reinterpreting :strided views is mathematically undefined." op src-elem new-elem)
               :source-location location)))))
