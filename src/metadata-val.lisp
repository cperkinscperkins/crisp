...
(define-forward-only-validator validate-089-09-global-size-exact-tiled (path)
  "Validates :global-size with :strategy :exact and :tile-shape (16 16) in metacrisp."
  (let ((k (or (%089-find-kernel path) (return-from validate-089-09-global-size-exact-tiled nil))))
    (let ((gs (%089-check-dispatch-key k :global-size "GLOBAL-SIZE")))
      (unless gs (return-from validate-089-09-global-size-exact-tiled nil))
      (unless (%089-decl-strategy= gs "EXACT")
        (log:error "Expected :strategy :exact in ~a" gs)
        (return-from validate-089-09-global-size-exact-tiled nil))
