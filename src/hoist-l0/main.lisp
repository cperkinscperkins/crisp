  ...
  (let* ((profile (metacrisp-hardware-profile data))
    ...
    (setf *l0-register-modes*
          ...
          *l0-compute-units*
          ...
          ;; 175: consumed by the symbolic scratch-size resolver.
          *l0-simd-width*
          (getf profile :simd-width))
    profile))
  ...
  (let ((allocations '()))
    ...
    (when (and *mma-bench-iters* *mma-test-dims*)
      (destructuring-bind (m n k) *mma-test-dims*
        ...
        (format stream "            bool _gotTs = false;~%")
        (format stream "            if (_tsOk) {~%")
        ...
        (format stream "                if (zeEventQueryKernelTimestamp(_tsEvent, &_ts) == ZE_RESULT_SUCCESS) {~%")
        ...
        (format stream "                    _gotTs = true;~%")
        (format stream "                }~%")
        (format stream "            }~%")
        (format stream "            if (!_gotTs) {~%")
...
;;; --- symbolic scratch sizes (175) -------------------------------------------
;;;
;;; A scratch tensor may be sized with a KEYWORD rather than an integer --
;;; (make-scratch-vector float :match-num-warps-per-workgroup).  The compiler passes that keyword
;;; through verbatim into the metacrisp as :size-expr, by design, and the HOISTER resolves it.
;;;
;;; WHY HERE AND NOT IN THE COMPILER.  Crisp is a kernel-only language with no runtime.  The
;;; generated .cpp is SAMPLE host code the user may adapt or discard, and the real global/local
;;; sizes are chosen by whoever performs the enqueue.  The (local-size ...) declaration is author
;;; INTENT -- used to fit and comment the generated launcher -- not a fact the compiler may fold
;;; against.  Folding the size to a literal at compile time would bake in an assumption the host is
;;; free to violate: a kernel handed a 4-element scratch buffer could be launched with 8 warps and
;;; corrupt silently.
;;;
;;; Hence the size is emitted as a C++ EXPRESSION over named geometry constants, not as a number.
;;; Editing wg_size in the generated launcher re-sizes every dependent scratch buffer, which is the
;;; entire point of a symbolic size; a baked literal would reduce :match-num-warps-per-workgroup to
;;; "a comment that computed a 4 at build time".
...
(defun %l0-scratch-dims (size-expr rank param-name)
  ...
  (cond
    ((%l0-scratch-symbolic-size-p size-expr)
     (if (= rank 1)
...
(defun %elem-type-bytes (elem-str)
  (cond
...
(defun %l0-emit-local-scratch-tensor-arg (stream param param-name param-type arg-index)
  ...
  (let ((size-expr (getf param :size-expr))
    ...
    (if (and (%l0-scratch-symbolic-size-p size-expr) (= rank 1))
        ...
        (%l0-emit-concrete-local-scratch-tensor-arg
         stream param param-name param-type arg-index))))

(defun %l0-emit-concrete-local-scratch-tensor-arg (stream param param-name param-type arg-index)
...
;;; --- launch geometry constants (175) ----------------------------------------
;;;
;;; Emitted for EVERY kernel, not only those with symbolic scratch, so that %l0-emit-dispatch can
;;; always phrase zeKernelSetGroupSize in terms of them.
;;;
;;; They are emitted just BEFORE the argument-setup block.  That matters: kernel arguments are set
;;; well ahead of zeKernelSetGroupSize in the generated main(), and the two regions share one brace
;;; scope, so constants declared here are visible to both.
...
;;; size each dispatch strategy chose, read off the launch call.  Moving the value into a constant
;;; relocates the observable those specs were written around, so the directives would have to be
;;; rewritten to assert the constant instead.  That is a deliberate change to what the suite pins,
;;; not a side effect of this endeavour, so it is left alone.
;;;
;;; CONSEQUENCE, stated plainly: the workgroup size appears TWICE in the generated launcher -- once
;;; as wg_size_x/wg_size_y here, once as literals in zeKernelSetGroupSize.  A user who retunes one
;;; and not the other gets a scratch buffer sized for the old geometry.  The comment emitted in the
;;; geometry block names the other site so the edit is at least discoverable.  Closing the gap
;;; properly means updating those nine directives.
...
(defun generate-kernel-arguments-with-usm (stream declared-sig aliases records context-var device-var dispatch-info)
  ...
  ;; 175: the launch-geometry constants the symbolic scratch sizes are phrased against.
  ;; Emitted FIRST, and for every kernel -- two unused block-scope consts cost nothing in C++
  ;; and keep one launch geometry in one place.
