...
- [~] `(with-precision (<KEY>) ...)`  (pass 5a DONE 2026-07-02 — FORWARD: new semantic-with-precision node (semantic.lisp) whose codegen dynamically binds *math-precision* over just its body (force-lock respected); analyze-with-precision-expression (control.lisp) registered under both packages like warp builtins; added the node to the semantic-node-type / semantic-node-source-location etypecases. Precedence force > with-precision > declaim > math. Specs 14 (fast region in ieee baseline), 15 (ieee region in fast baseline), 16 (force beats region) green — IR-verified per-region mixing (fmul fast beside fmul float in one kernel); regression 806/806 both ways (14–16 SKIP-WITH[--differentiate]), 253 unit, 183 neg. **5b (with-precision × AD) PENDING — backward must reproduce the per-region scoping; then un-SKIP 14–16.**)
