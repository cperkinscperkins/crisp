...
- [x] `(with-precision (<KEY>) ...)`  (pass 5a DONE 2026-07-02 — FORWARD: new semantic-with-precision node (semantic.lisp) whose codegen dynamically binds *math-precision* over just its body (force-lock respected); analyze-with-precision-expression (control.lisp) registered under both packages like warp builtins; added the node to the semantic-node-type / semantic-node-source-location etypecases. Precedence force > with-precision > declaim > math. Specs 14 (fast region in ieee baseline), 15 (ieee region in fast baseline), 16 (force beats region) green — IR-verified per-region mixing (fmul fast beside fmul float in one kernel). **5b (with-precision × AD) DONE 2026-07-02 — the region is transparent to the derivative STRUCTURE (chain rule is precision-independent), so the AD/backward pipeline strips it: anf-transform.lisp normalizes `(with-precision (K) body)` as its body (single form ANF'd directly; multi-form → progn) so the backward walk never sees the wrapper. The FORWARD kernel keeps its region precision (semantic-with-precision codegen untouched — verified under --differentiate too via 14–16); only the backward ops fall back to ambient precision (correct gradient value; region-precision-in-backward is a deferred refinement, only affects fast-region numerical verify which is untested). Un-SKIPPED 14–16 under --differentiate (all green) + new spec 17 (VERIFY-AUTODIFF: (with-precision (ieee) (* x x)), dres/dx=2x → analytical 6.0 vs numerical 5.9996 @ x=3). Regression: 807/807 both ways, 253 unit, 183 neg.**)
...
- [x] Warning + error test coverage (DONE 2026-07-02): the two CLI *warnings* now have tests via a new `EXPECT-STDERR[flags]: "substring"` directive (parse-expect-stderr + run-spec-expect-stderr-pass, step 2.5 of run-spec-file: compile with flags, PASS iff exit 0 AND substring on stderr; warnings are raw `format` to *error-output*, so `--log-level=off` doesn't hide them). Spec 18 (--force-math-precision override warning), spec 19 (fast+preserve not-honored warning). The two source-level bad-key ERRORS get CHECK-FAIL negatives under 126-precision/errors/: 01 (with-precision bad key), 02 (declaim bad key). NOTE gaps: (a) fast+preserve warning is flag-level only — declaim/with-precision `fast` + `preserve` doesn't warn; (b) the unknown-CLI-*value* errors (--math-precision=bogus etc.) exit≠0 but the negative runner can't inject per-test flags, so they're uncovered. tests.md documents EXPECT-STDERR. Suite 811/811 both ways, 253 unit, 185 neg.
