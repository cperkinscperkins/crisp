...
Important: These helpers support a `:barrier` key for asynchronous execution.  See [Topologically Aware Async](#topologically-aware-async) for more information. 
...
```
(load-tile-at source-tensor dest-tile (... tensor-row-y tensor-col-x) &key (identity 0) transpose  barrier)


(store-tile-at source-tile dest-tensor  (... tensor-row-y tensor-col-x) &key transformF transpose barrier)
...
are availalable in the body of the `tile-stride` (along with async variants). See [load-tile / store-tile](#load-tile) for a full discussion.

Outside that macro, tile loading and storing is available, but coordinates are needed. 
`load-tile-at` and `store-tile-at` can be used.  
...
workgroups the hardware guarantees are co-resident on one GPC, close enough to address one
another's local memory and share a barrier.  `cluster-size` declares how many workgroups make up
one cluster, and in what shape.

> **The value counts WORKGROUPS, not threads.**  Every sibling declaration here — `global-size`,
> `local-size` — is measured in threads.  This one is not.  `(cluster-size :set-to 2)` means *two
> workgroups*, however many threads each contains.  `(cluster-size :set-to 256)` is not a large
> cluster; it is a request the hardware will refuse.

Two capabilities follow: **Distributed Shared Memory**, where a workgroup can read and write its
peers’ `:local` memory and `sync-cluster` spans the group; and **multicast tile loads**, where one
fetch from global memory lands in every member’s local memory at once.  The second is why
`cluster-size` exists today.  It makes multicast *possible*; an individual `load-tile` asks for it
with `:multicast`.  See **Clusters and Distributed Shared Memory** in the topology material for
the multicast rules.

**Axes follow `:tile-shape`** — axis 0 tracks dimension 0, so for a row-major output tile grid
axis 0 is rows and axis 1 is columns.  The rank of `cluster-size` must match `:tile-shape`; axes
beyond the declared rank are 1, and a scalar is shorthand for rank 1.

**Unlike its siblings, this declaration is not advisory.**  `global-size` and `local-size` shape
the hoisting code and leave the kernel untouched; `cluster-size` alters code generation — the
multicast destination mask, which workgroup issues a multicast load, and the cluster entry/exit
fences — and the cluster dimensions are recorded in the generated PTX so the driver enforces the
host/kernel agreement at launch.  Two consequences:

- **There is no `:derive-from`.**  A shape baked into code generation cannot come from a host-side
  runtime value.
- **The portable maximum is 8 workgroups.**  Larger clusters exist on some parts (16 on Hopper)
  but need an explicit non-portable opt-in.

**Grid divisibility.**  The grid dimensions must divide by the cluster dimensions — the driver
rejects an indivisible launch outright, per axis.  Under `:strided` the compiler pads the grid up
to a cluster multiple and notes it in the hoisting comments; the extra workgroups find no tiles
left to claim and exit.  Under `:exact` there is no stride loop to absorb the padding, so an
indivisible grid is a hard compile error naming the tile shape and cluster shape that conflict.

**Degradation.**  On Intel or pre-Hopper NVIDIA the cluster extent drops to 1 and multicast loads
become ordinary per-workgroup loads.  The kernel still computes the right answer, so Crisp emits a
diagnostic rather than degrading silently, and records the effective extent in the kernel
metadata.  (A `:mode :cluster` *barrier* is a different matter — that is a compile error on
SPIR-V, not a degrade.)

**`:msg`** — as with `global-size` and `local-size`, a string emitted as a comment where the
...
> **This is the high-level convenience form, and it is not the fast path.**  For tensor-core /
> DPAS matrix multiply — register tiles, `mma-accumulate-via-tile`, async staging, rings, warp
> specialization and both vendor optimization arcs — see
> [Matrix Multiplication](#matrix-multiplication) under Topologically Aware Compilation below.
...
The two synchronization entries above are the fused, workgroup-wide forms.  Crisp also has
`sync-warp` and `sync-cluster`, a split `:arrive` / `:wait` form of `sync-workgroup`, and the
arrival-counting `make-arrival-sync` family.  All of them, with their per-backend lowerings, are
documented under **Sync Operations** in the topology material below.
...
Names a hardware profile for the compiler to optimize and validate against.  `<NAME>` is either
one of the profiles Crisp ships (`bmg`, `h100`) or a profile defined with `def-hardware-profile`
in one of the `.crisp` files in this invocation.

A profile is always optional — `--ir-target` alone compiles a correct kernel — but several forms
consult it and a few refuse without it.  See **Hardware Profiles** for what a profile holds and
which forms read it.

It is an error to use this flag if a topology or orchestration already specifies a profile.
